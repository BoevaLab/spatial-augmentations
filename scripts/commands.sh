...
python src/train.py task_name=augmentation test=True tags="[train+test, baseline]" data.processed_dir=data/domain/processed_knn_baseline data.redo_preprocess=True data.batch_size=1 data.augmentation_mode=baseline model.augmentation=baseline model.processed_dir=data/domain/processed_knn_baseline
